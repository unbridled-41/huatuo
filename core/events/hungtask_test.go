...
func TestHungTaskUpdateDataNotSharedAcrossScrapes(t *testing.T) {
	attr, err := newHungTask()
	if err != nil {
		t.Fatalf("newHungTask() error = %v", err)
	}
	c := attr.TracingData.(*hungTaskTracing)
	...
	atomic.AddInt64(&hungtaskCounter, 5)
...
func TestHungTaskUpdateConcurrentScrapes(t *testing.T) {
	attr, err := newHungTask()
	if err != nil {
		t.Fatalf("newHungTask() error = %v", err)
	}
